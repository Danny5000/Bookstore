...
import { planRefundAllocationComponents } from './refund-allocation-components';
import {
...
export interface RefundFulfillmentDependencies {
  ...
  projectEntitlement?: RefundEntitlementProjector;
...
export async function fulfillRefundEvent(
  ...
  const dependencies = {
    ...
    projectEntitlement: dependencyOverrides.projectEntitlement,
  ...
  await database.transaction(async (transaction) => {
    ...
    if (allocation.state === 'allocated' || allocation.state === 'noop') {
      ...
      const access = await recomputeRefundPurchaseAccess(transaction, {
        items,
        allocations: allAllocations,
        grants,
        now
      }, dependencies.projectEntitlement);
      const accessChangedScopes = access.projectedScopes.filter(
        (scope) => scope.beforeActive !== scope.afterActive
      );
