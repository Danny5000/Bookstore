...
**Status:** Plan 6B complete; Plan 7A Checkpoint C backend authority implemented

The completed unified Plan 6B implementation includes ingestion/reconciliation and the direct administrator Sales, review, payout, correction, recovery, and CSV surfaces. The protected global Sales link is live for authorized administrators. Plan 7A Checkpoints A and B added dependency/test boundaries, structured logging, diagnostic correlation, and worker freshness; Plan 7A Checkpoint C adds backend operations authority, but Plan 7A remains incomplete. At current head, the migration chain ends at `0015_plan7a_operations_authority`, and the executable verifier is `plan7a-database-catalog-v1`. Historically, `0012` retained eight callable public boundary routines, `0013` added the ninth, and `0014` changed no callable surface while fail-closing the nullable issue-transition guard. Production remains maintenance-only and Stripe-disabled. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md).
...
| `WORKER_READY_FILE` | `.worker-ready`; worker and one-shot health only | Compose fixes `/tmp/worker-ready`; worker and one-shot health only | No |
| `WORKER_CONCURRENCY` | `.env` value `1`; worker and one-shot health only | Deployment-process environment or Compose default `1`; worker and one-shot health only | No |
| `WORKER_HEARTBEAT_INTERVAL_MS` | default `5,000` milliseconds; worker and one-shot health only; integer between `1,000` and `30,000` milliseconds | Deployment-process environment or Compose default `5,000`; same worker/health scope and bounds | No |
| `WORKER_HEARTBEAT_MAX_AGE_MS` | default `20,000` milliseconds; worker and one-shot health only; must satisfy `WORKER_HEARTBEAT_MAX_AGE_MS >= 3 * WORKER_HEARTBEAT_INTERVAL_MS`, `WORKER_HEARTBEAT_MAX_AGE_MS >= JOB_POLL_INTERVAL_MS + 2 * WORKER_HEARTBEAT_INTERVAL_MS`, `WORKER_HEARTBEAT_MAX_AGE_MS < JOB_LEASE_MS`, and `WORKER_HEARTBEAT_MAX_AGE_MS <= 300000` | Deployment-process environment or Compose default `20,000`; same worker/health scope and constraints | No |
...
The one-shot health loader reads only six nonsecret values: `WORKER_READY_FILE`, `WORKER_CONCURRENCY`, `WORKER_HEARTBEAT_INTERVAL_MS`, `WORKER_HEARTBEAT_MAX_AGE_MS`, `JOB_POLL_INTERVAL_MS`, and `JOB_LEASE_MS`. It does not load the application schema, database settings, storage, SMTP, authentication, or Stripe configuration. Web, migration, role provisioning, bootstrap, and storage cleanup do not read or retain the worker heartbeat settings. In fully containerized development their inherited direct and `_FILE` forms are explicitly empty; production omits them from those processes.

Every required value also supports a mutually exclusive `<NAME>_FILE` form. Production uses `DATABASE_OWNER_PASSWORD_FILE`, `DATABASE_PASSWORD_FILE`, `DATABASE_WORKER_PASSWORD_FILE`, `DATABASE_STORAGE_CLEANUP_PASSWORD_FILE`, `AUTH_SECRET_FILE`, and `SMTP_PASSWORD_FILE` under `/run/secrets`; the bootstrap tool also uses `BOOTSTRAP_ADMIN_PASSWORD_FILE`. The owner secret reaches only PostgreSQL, migration, and role provisioning. Migration loads database settings only. Bootstrap receives the web database login and bootstrap identity inputs only. The app uses the web login, the worker alone uses the worker login and SMTP credentials, and storage cleanup receives only its dedicated login. Role provisioning is the sole application service that receives all four database credentials. The Stripe overlay gives the app its webhook-verification secret and gives both app and worker the API secret; the worker never receives the webhook secret. Each scoped loader fails when one of its own values is missing, empty, invalid, or supplied both directly and through `_FILE`, without reading secrets assigned to another process. The Plan 7A operations capability has no direct or `_FILE` configuration: each claim creates it in worker memory, exposes it only transaction-locally, and persists only its digest. Financial-administrator and revision-ingestion authority stay separate.

## Production baseline

Keep the evidence order exact: migrate through `0015_plan7a_operations_authority`, provision and attest the four pairwise-distinct owner/web/financial-worker/storage-cleanup principals, capture the checkpoint, rehearse it on a distinct database engine, then run the production-image smoke. Every checkpoint and restore consumer must authenticate the `plan7a-database-catalog-v1` executable verifier. The web principal uses complete owner-controlled routines; protected financial and operations mutations remain worker-only and require their separate current authority.
...
Plan 7A Checkpoint C exposes only a backend operations list/submit/owned-status surface. Application authorization requires `jobs.retry` before input inspection, and complete database routines perform current-role reauthorization under the administrator lock. The exhaustive catalog has eleven registered production kinds, but only the two local rearm policies are enabled and neither calls a provider. There is no generic reset or operations UI. Exact command, audit, claim, and restore guards retain command history and keep the operations capability distinct from the financial and revision-ingestion paths.
...
`/health/live` proves that the Node process responds. `/health/ready` performs a bounded `select 1` through the web process's PostgreSQL pool, round-trips staging and covers with canonical disposable keys, and reads and byte-compares the fixed publication sentinel without writing to the read-only publication mount. Worker startup still performs its database probe, atomically provisions and verifies that sentinel, and uses canonical disposable keys to round-trip all three roots. It publishes no healthy evidence until those probes, every configured slot's first successful poll, and the first atomic heartbeat publication all succeed. Later checks require both publication and every slot's progress to remain fresh; missing, malformed, stale, too-far-future, wrong-slot-count, or missing-slot evidence is unhealthy. The validator is a stateless local process with no network endpoint or public response, and it does not read or require database credentials. The bounded cleanup process performs the same writer probe before scanning. Transient probe writes are deleted immediately, while a crash remnant remains an ordinary health, derived, or cover orphan that existing retention cleanup can remove. The sentinel is outside cleanup grammar and the current publication backup/restore contract requires and preserves it. All other production paths return 503 while `APPLICATION_MODE=maintenance`.

An unhealthy state alone does not terminate a process, and Compose does not restart a container merely because it is unhealthy. A heartbeat publisher failure is different: it aborts worker activity and exits nonzero after bounded cleanup, so production `restart: unless-stopped` can replace it. Fatal worker cleanup is capped at 10 seconds with a status-`1` force exit if it wedges; ordinary `SIGINT`/`SIGTERM` shutdown keeps the worker's 30-second Compose stop grace. Checkpoints B and C add no monitoring, alert transport, worker metrics route, or public worker-health response.
...
- Plan 6B combines completed processing-fee, balance-transaction, payout, allocation, issue, scheduler, and replay boundaries with administrator resolution and sales/estimated-payout reporting; its protected global Sales link is live.
- Plan 7A Checkpoints A and B add dependency/test boundaries plus structured logging, diagnostic correlation, and worker freshness. Plan 7A Checkpoint C adds the closed job catalog and backend operations authority. Operations UI, generalized stage and smoke evidence, fresh release-candidate capture, deployment automation and hardening, off-host backup scheduling, monitoring/alert delivery, final capacity tuning, production-live activation, Stripe enablement, Checkpoint D, and the read-only-rootfs review remain deferred.
