...
The completed unified Plan 6B implementation includes ingestion/reconciliation and the direct administrator Sales, review, payout, correction, recovery, and CSV surfaces. The protected global Sales link is live for authorized administrators. Production must remain `APPLICATION_MODE=maintenance` with Stripe-disabled defaults; Plan 7 owns production activation and operability, including general retry administration, monitoring, off-host backup scheduling, and deployment hardening. The migration chain ends at `0014`: `0012` retains the historical eight callable public boundary routines, `0013` adds the final ninth, and `0014` changes no callable surface while replacing the nullable issue-transition trigger guard with a fail-closed definition. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md).
...
- Plan 7 adds production activation and operability, including deployment automation and hardening, off-host backup scheduling, monitoring/alert delivery, general retry administration, final capacity tuning, and the read-only-rootfs review.
