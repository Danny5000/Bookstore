...
The completed unified Plan 6B implementation includes ingestion/reconciliation and the direct administrator Sales, review, payout, correction, recovery, and CSV surfaces. The protected global Sales link is live for authorized administrators. Plan 7A Checkpoints A and B have added dependency/test boundaries, structured logging, diagnostic correlation, and worker freshness, but Plan 7A is not complete. Production must remain `APPLICATION_MODE=maintenance` with Stripe-disabled defaults. The migration chain ends at `0014`: `0012` retains the historical eight callable public boundary routines, `0013` adds the final ninth, and `0014` changes no callable surface while replacing the nullable issue-transition trigger guard with a fail-closed definition. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md).
...
- Plan 7A Checkpoints A and B add dependency/test boundaries plus structured logging, diagnostic correlation, and worker freshness. General retry administration and operations UI, generalized release/smoke evidence, deployment automation and hardening, off-host backup scheduling, monitoring/alert delivery, final capacity tuning, activation, Stripe enablement, and the read-only-rootfs review remain deferred.
