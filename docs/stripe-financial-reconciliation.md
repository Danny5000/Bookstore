...
Every result must be zero. These are cross-row service invariants in addition to database checks.
...
Every structural result except the two `failed_running_scan_*` rows must be zero. Those final rows are separate operational evidence: `failed_running_scan_retry_exhausted` identifies exhausted retryable work, while `failed_running_scan_permanent` identifies a bounded permanent failure before its attempt ceiling. The general failed-job procedure remains Plan 7 work and 6B-I provides no SQL reset or arbitrary retry. A `succeeded` current identity for a running scan is structural failure because the permanent key can no longer resume it.
...
## Candidate gate evidence (2026-08-13)

This is evidence for the committed review candidate, not checkpoint completion. The status remains **6B-I candidate — independent review pending; 6B-II pending** until review findings are resolved, every gate is rerun on the final tree, and all five final reviewers clear that exact commit.
...
- `npm run check`, `npm run lint`, and both production builds passed. The unit gate passed 1,536 tests across 175 files; the PostgreSQL integration gate passed 427 tests across 48 files; and all 12 Chromium journeys passed. The four release-contract suites passed 109 tests across four files, including production-smoke ownership, privacy, operations, and fixture-runtime contracts.
...
- The standalone production smoke built and verified a 138,554,299-byte image with digest `sha256:9c6f3a906593a4ff56a1ab13bb62a0bfa7c6bdd12953f98529693d6be55d19b7`. It ran migrations twice without drift, verified the exact classifier/allocation projection seed and disabled replay root, brought the maintenance-mode topology healthy, proved Stripe credentials and mounts absent, and removed its owned Compose resources and image.
- The fixture-runtime smoke independently built a same-source 138,554,299-byte candidate image with digest `sha256:66c04e3d0e5ac771221e1fa2c2658c9b4d28efe80fe2d36b03f43184dc5c972c`. Without external Stripe access or real credentials, it accepted one order and one checkout session through the actual web path and observed one completed financial scan in the worker. It then removed the fixture alias, candidate image, containers, network, volumes, secrets, and temporary manifests. A post-gate Docker inventory contained no owned smoke or upgrade resources.
