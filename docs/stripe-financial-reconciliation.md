...
### Post-restore executable schema-object check

This canonical, versioned manifest is executable restore evidence for every protected migration journal entry with `idx >= 7` through current. It rejects missing, additional, or catalog-different protected tables, views, enum labels, touched legacy columns, routine signatures and definitions, triggers, constraints, and explicit indexes. It also rejects the two retired enum types and three retired reconciliation columns. The contract pins physical table/view kind and persistence, row-security state, created-object owners, exact non-owner direct ACLs including normalized grantors, and the effective `STORAGE_CLEANUP_LOGIN` group/login authority boundary. Each protected table fingerprints sorted full constraint, inbound-foreign-key, non-constraint-owned index, noninternal trigger, rewrite-rule, and inheritance-edge inventories. Constraint-owned indexes are represented through their constraints, with the primary key repeated as a dedicated table-shape field, while every explicit `CREATE INDEX` remains an independent catalog object.

The four legacy relations in the 0010 direct-ACL boundary (`outbox_messages`, `guest_identities`, `entitlement_grants`, and `entitlements`) additionally pin ordinary-table, permanent-storage, and disabled-RLS state. Their owners are intentionally treated as part of the trusted database-owner/restore boundary because the migrations do not canonicalize legacy ownership; their non-owner ACL grantors are still exact. RLS policy rows are not separately inventoried because both RLS flags are pinned off, making policies behaviorally inactive.
...
### Post-restore source evidence projection check

This query binds each source evidence state to its exact current projection heads, canonical source principal, and open issue disposition. Pending sources may be unprocessed; exception sources require a current exception issue; fee-reconciled sources require complete gross and fee heads for every linked balance transaction. Same-currency payment charges and primary refund transactions must equal their signed source principal directly. A dispute binds the presentment rows of its first chronological withdrawal, and of any cross-currency withdrawal, to the immutable dispute amount; later same-currency withdrawals keep independent settlement conservation. Cross-currency settlement, refund-failure transactions, dispute reinstatements, and fee credits remain separate domains and are not misclassified as source-principal drift.
...
### Post-restore deterministic title allocation check

This query replays algorithm-version-1 largest-remainder allocation for immutable charge, refund, and failed-refund gross facts and rejects conservation-balanced per-title or per-component drift.
...
- The isolated 0006-to-0007 upgrade command passed all 10 owned valid/adversarial fixture outcomes: valid, zero-refund, zero-allocation, zero-dispute, over-allocation, currency-conflict, partial-facts, pending-refund-allocation, failed-refund-allocation, and canceled-refund-allocation. The valid path advanced the migration journal from seven entries to eight exactly once and proved a second migration run is a no-op; rejected histories rolled back without advancing the journal. Every owned PostgreSQL container, network, and volume was removed afterward.
- The standalone production smoke built and verified a 138,620,203-byte image with digest `sha256:81c6ed537f456f4dd30c5f350e768b2d1fc81e30e9ed462dc037acf426d64af4`. It ran migrations twice without drift, verified the exact classifier/allocation projection seed and disabled replay root plus its terminal finalizer, brought the maintenance-mode topology healthy, proved Stripe credentials and mounts absent, and removed its owned Compose resources and image. The first fresh attempt exited during runtime startup/readiness after building and migrating; its owned resources were removed, and two consecutive authoritative reruns then passed without a code or configuration change. The digest above is from the final rerun.
- The fixture-runtime smoke independently built a same-source 138,620,203-byte image with digest `sha256:bdc0e73e6f27ef103e1e86b66a0ebf9340467b58a78d5158acd0bc23810a05a0`. Without external Stripe access or real credentials, it accepted one order and one checkout session through the actual web path and observed one completed financial scan in the worker. It then removed the fixture alias, image, containers, network, volumes, secrets, and temporary manifests. A fresh post-gate Docker inventory contained no owned smoke or upgrade containers, networks, volumes, or images. Seven older `%TEMP%\pale-orbit-plan6b-upgrade-*` directories that predated this final gate were observed and deliberately left untouched; no current gate-owned temporary artifact remained.
