...
A maintenance fixture may run from a dirty developer tree, is always marked non-release, and cannot satisfy a later activation input. Its `sourceMode` is `workspace_fixture`; it records the starting `HEAD` as an informational `sourceRevision`, records the actual `sourceClean` boolean, and materializes an exact frozen workspace context containing tracked modifications and untracked nonignored inputs after applying the workspace `.dockerignore`. A release candidate requires a clean Git worktree including no untracked nonignored files. Its `sourceMode` is `committed_revision`, `sourceClean` is true, and the runner exports the exact `HEAD` tree. Both modes construct a fresh, initially empty logical build context and admit only the selected source blobs and selected Dockerfile after Docker-compatible ignore evaluation. Checkpoint D encodes that context directly as deterministic POSIX ustar bytes and streams it to Docker stdin; “build root” does not require or permit a host build directory. Inputs later needed by Compose or checkpoint operations are materialized separately beneath platform-private restricted-path leases. Both modes reject submodules, unsupported file modes, path escapes, missing build inputs, or a source change during snapshotting. Ignored worktree content cannot enter either build.
