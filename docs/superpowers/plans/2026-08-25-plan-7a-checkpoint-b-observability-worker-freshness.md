...
On open/write/sync/close/rename failure, remove only the temp path, preserve the previous good target where the operating system has done so, reject `run`, and reject an unresolved `firstHealthyPublication`. Do not retry, generate alternate temp names, truncate the target in place, or log the record/path/error.
