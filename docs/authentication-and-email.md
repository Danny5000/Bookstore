...
The completed unified Plan 6B implementation adds direct administrator financial review and audited command workflows without changing the authentication or email authority below. At current head, the migration chain ends at `0015_plan7a_operations_authority`, and the executable verifier is `plan7a-database-catalog-v1`. Historically, `0012` retained eight callable public routines, `0013` added the ninth, and `0014` changed no callable surface while replacing the nullable issue-transition trigger guard with a fail-closed definition; Plan 7A Checkpoint C adds the separate operations-command authority in `0015`. The exact four pairwise-distinct principals remain `DATABASE_OWNER_USER`, `DATABASE_USER`, `DATABASE_WORKER_USER`, and `DATABASE_STORAGE_CLEANUP_USER`: the web principal uses only complete owner-controlled boundaries, while the worker executes protected mutations under task-specific authority. The required evidence order remains migrate → provision → checkpoint capture → distinct-engine rehearsal → smoke. The protected global Sales link is live, but production remains maintenance-only and Stripe-disabled. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md).
...
Production must remain `APPLICATION_MODE=maintenance` after Plan 7A Checkpoint C. Catalog, storage, commerce, guest claims, customer libraries, full entitled reading, original downloads, and Plan 6B financial reporting are durable. The protected global Sales link is live, while operations UI, monitoring and alert delivery, generalized release evidence, off-host backup scheduling, deployment hardening, production activation, Stripe enablement, and Checkpoint D remain deferred.
...
Do not manually reset failed email rows or jobs. Plan 7A Checkpoint C provides only a backend operations list/submit/status service governed by `jobs.retry` and current-role reauthorization; it adds no route or retry control. Its administrator retry policy for `outbox.dispatch` is disabled; automatic outbox delivery and bounded worker retries remain active, a delivered outbox cannot be redelivered, and no generic reset exists. Correct the underlying SMTP/configuration problem and preserve the failed records and command history for review.
