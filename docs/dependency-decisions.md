...
Plan 6B-I and 6B-II form one completed implementation and add no production runtime dependency beyond the decisions below. Plan 7A Checkpoint C likewise adds no production package or provider/network edge: the selected Node runtime's built-in cryptography generates each fresh per-claim operations capability, and PostgreSQL `pg_catalog.sha256` computes and verifies the persisted digest. The clear capability is not an environment secret. At current head, the migration chain ends at `0015_plan7a_operations_authority`, and the executable verifier is `plan7a-database-catalog-v1`. Historically, `0012` retained eight callable public boundary routines, `0013` added the ninth, and `0014` changed no callable surface while replacing the nullable issue-transition trigger guard with a fail-closed definition. The four pairwise-distinct principals and the separation between web, worker, financial-admin, revision-ingestion, and operations authority remain intact. Checkpoint C registers exactly eleven production job kinds and enables only two local no-provider-call rearm adapters; it adds no route or user interface. Production remains maintenance-only and Stripe-disabled, while monitoring, release coordination, off-host backup scheduling, deployment hardening, activation, Stripe enablement, and Checkpoint D remain deferred. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md).
