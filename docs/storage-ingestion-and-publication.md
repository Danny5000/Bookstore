...
**Status:** Plan 6B implementation complete; Plan 7A Checkpoint C backend operations authority complete; Checkpoint D deferred

The migration chain ends at `0015_plan7a_operations_authority`, and the executable verifier is `plan7a-database-catalog-v1`. Historical migration `0012` retains its eight callable financial-administration routines, historical `0013` adds the ninth, and historical `0014` changes no callable surface while replacing the nullable issue-transition trigger guard with a fail-closed definition. The exact pairwise-distinct database principals remain `DATABASE_OWNER_USER`, `DATABASE_USER`, `DATABASE_WORKER_USER`, and `DATABASE_STORAGE_CLEANUP_USER`; web submit/status/audit authority remains separate from worker mutation authority.

Plan 7A Checkpoint C adds only the closed backend job-retry authority. It adds no operations route, page, navigation, polling, or button, makes no provider call, and leaves financial-administration and revision-ingestion authority separate. The required release-evidence sequence remains migrate → provision → checkpoint capture → distinct-engine rehearsal → smoke, and the current 14-artifact checkpoint contract remains indivisible. Production stays maintenance-only and Stripe-disabled. Monitoring/alerts, generalized stage evidence, production-live activation, Stripe enablement, fresh release-candidate capture, and Checkpoint D remain deferred, along with off-host backup scheduling and deployment hardening. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md) and [database and worker operations](database-and-workers.md).
...
Database, storage, timeout, and worker-abort failures are transient. PostgreSQL jobs retry them automatically with bounded backoff until `maxAttempts`. When a revision is in `failed` and its staged source still matches the recorded size and SHA-256 checksum, **Retry ingestion** creates a new generation and job. This is a specialized revision-ingestion workflow with its own authority and immutable-source checks: the Checkpoint C policy for `catalog.ingest_revision` is fixed disabled, so no general ingestion retry exists through operations authority. If the source is absent or mismatched, retry is disabled and a new upload is required. Never edit an original or derived object in place.
...
The rehearsal pre-migrates through `0015_plan7a_operations_authority`, restores `database.dump` with ownership suppressed but authenticated ACLs preserved, post-migrates the restored journal through that same current head, provisions roles, restores the split volumes, and exact-compares the exhaustive migration journal, every ordinary or partitioned base table in `public` and `drizzle`, the complete storage-reference inventory, and verifier output. The authenticated copied verifier must contain the calibrated `plan7a-database-catalog-v1` contract. It retains the prior financial command, owner-only `financial_admin_job_claims`, correction, fail-closed trigger, routine, and ACL evidence while adding exact operations command/claim/catalog/routine/trigger/GUC authority. The dynamic row-count inventory includes `operations_job_retry_commands` and `operations_job_retry_claims`; command history is retained, only per-claim capability digests persist, and no clear capability may appear in the database or any restore artifact. The rehearsal proves every restored database reference still matches the authenticated bytes and digest in its routed manifest, runs storage cleanup in dry-run mode, and checks maintenance liveness/readiness only after those proofs. The source bundle remains unchanged.
...
This legacy rollback procedure uses the bundle-authenticated historical Plan 6B contract, not the current Checkpoint C head or verifier. Run committed migrations once against the empty target through `0014_plan6bii_issue_transition_fail_closed` so the cluster-global `pale_orbit_*` group roles referenced by dump ACLs exist. Restore the authenticated dump with ACLs intact, run committed migrations again against the restored journal/schema through that same legacy head, and only then run the bundle's `database-role-provision` to bind the four distinct login roles. Both migration passes use the validated non-secret login-name attestation required by 0012 and preserved by 0013 and 0014. The authenticated copied legacy restore verifier derives its protected migration range from journal entries with `idx >= 7` through that bundle head and must contain the calibrated historical `plan6b-financial-catalog-v4` exact catalog contract, including the command and owner-only `financial_admin_job_claims` tables, private claim helpers, changed job authority, the correction-only issue resolver, the fail-closed issue-transition trigger definition, exact routine/default ACLs, and absence of a persisted clear capability token. It separately rejects any excess effective authority inherited by the normalized `STORAGE_CLEANUP_LOGIN`. The bundle's `restore-row-counts.csv` remains a catalog-derived inventory of every ordinary or partitioned base table in `public` and `drizzle`, not a hand-maintained table list. Do not replace this sequence with `--no-acl`: a restored journal can make migrations no-op and silently lose required grants. Keep the worker stopped throughout both migration passes, restore, role provisioning, and verification.
