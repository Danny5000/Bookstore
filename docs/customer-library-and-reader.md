...
The completed unified Plan 6B implementation preserves the access authority described here while adding the narrowly authorized refund-finalization and administrative-recovery workflows. The migration chain ends at `0015_plan7a_operations_authority`, and the executable verifier is `plan7a-database-catalog-v1`. Historical `0012` retains its eight callable financial routines, historical `0013` adds the ninth, and historical `0014` changes no callable surface while replacing the nullable issue-transition trigger guard with a fail-closed definition. The four pairwise-distinct principals remain `DATABASE_OWNER_USER`, `DATABASE_USER`, `DATABASE_WORKER_USER`, and `DATABASE_STORAGE_CLEANUP_USER`; the web principal submits and observes owner-scoped financial commands and records route audit evidence, while the worker alone mutates the protected financial/access graph.

Plan 7A Checkpoint C adds backend-only job-retry list/submit/status authority, but it cannot grant, revoke, or replace an entitlement and adds no library, reader, download, or operations UI. Its `jobs.retry` authorization and current-role reauthorization remain separate from both financial-administration and revision-ingestion authority. Production stays maintenance-only and Stripe-disabled. Monitoring/alerts, generalized stage evidence, production-live activation, Stripe enablement, fresh release-candidate capture, and Checkpoint D remain deferred, along with off-host backup scheduling and deployment hardening. See [financial reconciliation and reporting](financial-reconciliation-and-reporting.md) and [database and worker operations](database-and-workers.md).
...
Plan 6A adds the commerce/access chain that must be restored with that projection: `orders`, `order_items`, `payments`, `refunds`, `refund_allocations`, `disputes`, `stripe_events`, `entitlement_grants`, `guest_identities`, jobs, outbox messages, and audit events. It also adds security-critical `credential_authority`, which must be restored atomically with Better Auth users/accounts/verification state and checked only through aggregate missing/mismatch counts; never project its authorized hash or reset epoch. Checkpoint C adds `operations_job_retry_commands` and `operations_job_retry_claims` to the same indivisible restore evidence. Command, audit, and restore authority is exact, and command history is retained. Only a capability SHA-256 digest may persist with a claim; the clear capability remains memory/transaction-local and must not enter a dump, artifact, log, or environment secret. After restore, verify every purchase grant references its order item, unclaimed grants have no user, effective entitlements agree with the complete active grant set, and no raw provider payload/card/address/secret column exists. Follow [commerce and guest-claim operations](commerce-and-guest-claims.md) and the current restore contract in [storage, ingestion, publication, and recovery](storage-ingestion-and-publication.md).
...
- **Unexpected loss of access:** inspect the effective entitlement, every purchase/preserved/administrative grant for the user/title pair, canonical refund/dispute state, and relevant audit events without selecting email, auth, provider, or outbox secrets. Do not hand-edit `entitlements`. Use administrative recovery only when locked provenance proves that the exact administrative finalization caused the claimed purchase grant’s revocation and the current compatible correction makes that exact user/title eligible; there is no generic replacement-grant operation. `jobs.retry` cannot grant or revoke access, replay a financial-administration command, or invoke the separate revision-ingestion retry workflow.

Production must remain fixed to `APPLICATION_MODE=maintenance` with Stripe disabled. Plan 6B financial reconciliation/reporting and the Checkpoint C backend operations boundary are complete, but the production activation and operability gate remains deferred; deploying this branch alone must not make production public.
