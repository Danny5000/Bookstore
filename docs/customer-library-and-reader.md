...
Production must remain fixed to `APPLICATION_MODE=maintenance`. Plan 6B financial reconciliation/reporting is complete and its protected global Sales link is live, but Plan 7 must complete the production activation and operability gate, including general retry administration, monitoring, off-host backup scheduling, and deployment hardening; deploying this branch alone must not make production public.
